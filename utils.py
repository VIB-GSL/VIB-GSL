...
if __name__ == '__main__':
    ...
    dataset = get_dataset('IMDB-MULTI')
    print(dataset)
    node_num = [data.x.shape[0] for data in dataset]
    edge_num = [data.edge_index.shape[1]/2 for data in dataset]
    print(len(edge_num))
    print(np.mean(node_num))
    print(np.mean(edge_num))
    # print(dataset.data.y)

    # loader = DataLoader(dataset, 3, shuffle=True)

    # for i in range(10):
    #     idx = np.random.choice([i for i in range(10)])
    #     print(idx)
